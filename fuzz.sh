...
afl-fuzz -i $TESTCASE_DIR -o $AFL_OUT_DIR -m none -t 10000 -Mmaster-0 -- /results/mruby @@ >/dev/null &
for i in $(seq 1 $SLAVE_COUNT); do
    echo "Starting slave $i"
    afl-fuzz -i $TESTCASE_DIR -o $AFL_OUT_DIR -m none -t 10000 -Sslave-$i -- /results/mruby @@ >/dev/null &
    sleep 1
done
